...
static void ggml_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor) {
    ...
    switch (tensor->op) {
        ...
        case GGML_OP_ACC:
            {
                ggml_compute_forward_acc(params, tensor->src[0], tensor->src[1], tensor->opt[0], tensor);
            } break;
        case GGML_OP_SUB:
            {
        ...
        case GGML_OP_SET:
            {
                ggml_compute_forward_set(params, tensor->src[0], tensor->src[1], tensor->opt[0], tensor);
            } break;
        case GGML_OP_CPY:
            {
        ...
        case GGML_OP_GET_ROWS_BACK:
            {
                ggml_compute_forward_get_rows_back(params, tensor->src[0], tensor->src[1], tensor->opt[0], tensor);
            } break;
        case GGML_OP_DIAG:
            {
        ...
        case GGML_OP_CONV_1D:
            {
                ggml_compute_forward_conv_1d(params, tensor->src[0], tensor->src[1], tensor->opt[0], tensor);
            } break;
        case GGML_OP_CONV_2D:
            {
                ggml_compute_forward_conv_2d(params, tensor->src[0], tensor->src[1], tensor->opt[0], tensor);
            } break;
        case GGML_OP_FLASH_ATTN:
            {
                ...
                ggml_compute_forward_flash_attn(params, tensor->src[0], tensor->src[1], tensor->opt[0], masked, tensor);
            } break;
        case GGML_OP_FLASH_FF:
            {
                ggml_compute_forward_flash_ff(params, tensor->src[0], tensor->src[1], tensor->opt[0], tensor->opt[1], tensor->opt[2], tensor);
            } break;
        case GGML_OP_FLASH_ATTN_BACK:
            {
                ...
                GGML_ASSERT(t == 0 || t == 1);
                bool masked = t != 0;
                ggml_compute_forward_flash_attn_back(params, tensor->src[0], tensor->src[1], tensor->opt[0], tensor->opt[1], masked, tensor);
            } break;
        ...
        case GGML_OP_CROSS_ENTROPY_LOSS_BACK:
            {
                ggml_compute_forward_cross_entropy_loss_back(params, tensor->src[0], tensor->src[1], tensor->opt[0], tensor);
            }
            break;
        case GGML_OP_NONE:
            {
                ...
                case GGML_OP_ACC:
                    {
                        ...
                        if (ggml_is_quantized(node->src[0]->type)) {
                            cur = GGML_TYPE_SIZE[GGML_TYPE_F32] * node->src[1]->ne[0] * n_threads;
                        }
                ...
                case GGML_OP_FLASH_FF:
                    {
                        ...
                        if (node->src[1]->type == GGML_TYPE_F32) {
                            cur  = sizeof(float)*node->src[1]->ne[1]*node->n_tasks; // TODO: this can become (n_tasks-1)
                            cur += sizeof(float)*node->src[1]->ne[1]*node->n_tasks; // this is overestimated by x2
                        }
