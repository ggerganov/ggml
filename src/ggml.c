...
struct ggml_tensor * ggml_conv_2d(
        ...
        int                  d0,
        int                  d1) {
    // winograd: kernel 3x3, stride = 1, dilatation = 1
...
struct ggml_tensor * ggml_pad(
    struct ggml_context * ctx,
    struct ggml_tensor  * a,
    int p0, int p1, int p2, int p3, bool asymmetric) {
    bool is_node = false;
    ...
    struct ggml_tensor * result = ggml_new_tensor_4d(ctx, a->type,
            a->ne[0] + p0 * (asymmetric ? 1 : 2),
            a->ne[1] + p1 * (asymmetric ? 1 : 2),
            a->ne[2] + p2 * (asymmetric ? 1 : 2),
            a->ne[3] + p3 * (asymmetric ? 1 : 2));

    ggml_set_op_params_i32(result, 0, asymmetric ? 1 : 0);
...
static void ggml_compute_forward_pad_f32(
    ...
    int p3 = asymmetric ? 0 : (dst->ne[3] - src0->ne[3]) / 2;
        ...
        for (int64_t i1 = ith; i1 < ne1; i1 += nth) {
            for (int64_t i0 = 0; i0 < ne0; ++i0) {
                for (int64_t i3 = 0; i3 < ne3; ++i3) {
                    ...
                    if (
                        (asymmetric || i0 >= p0 && i1 >= p1 && i2 >= p2 && i3 >= p3) &&
                        i0 < (ne00 + p0) && i1 < (ne01 + p1) && i2 < (ne02 + p2) && i3 < (ne03 + p3)) {
                        const float * src_ptr = (const float *)((char *) src0->data + (i3 - p3)*nb03 + (i2 - p2)*nb02 + (i1 - p1)*nb01 + (i0 - p0)*nb00);
