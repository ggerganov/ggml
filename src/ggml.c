...
static void ggml_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor) {
    ...
    switch (tensor->op) {
        ...
        case GGML_OP_ADD:
            {
                ggml_compute_forward_add(params, tensor->src[0], tensor->src1, tensor);
            } break;
        case GGML_OP_ADD1:
            {
                ggml_compute_forward_add1(params, tensor->src[0], tensor->src1, tensor);
            } break;
        case GGML_OP_ACC:
            {
                ggml_compute_forward_acc(params, tensor->src[0], tensor->src1, tensor->opt[0], tensor);
            } break;
        case GGML_OP_SUB:
            {
                ggml_compute_forward_sub(params, tensor->src[0], tensor->src1, tensor);
            } break;
        case GGML_OP_MUL:
            {
                ggml_compute_forward_mul(params, tensor->src[0], tensor->src1, tensor);
            } break;
        case GGML_OP_DIV:
            {
                ggml_compute_forward_div(params, tensor->src[0], tensor->src1, tensor);
            } break;
        case GGML_OP_SQR:
            {
        ...
        case GGML_OP_SILU_BACK:
            {
                ggml_compute_forward_silu_back(params, tensor->src[0], tensor->src1, tensor);
            } break;
        case GGML_OP_NORM:
            {
        ...
        case GGML_OP_RMS_NORM_BACK:
            {
                ggml_compute_forward_rms_norm_back(params, tensor->src[0], tensor->src1, tensor);
            } break;
        case GGML_OP_MUL_MAT:
            {
                ggml_compute_forward_mul_mat(params, tensor->src[0], tensor->src1, tensor);
            } break;
        case GGML_OP_OUT_PROD:
            {
                ggml_compute_forward_out_prod(params, tensor->src[0], tensor->src1, tensor);
            } break;
        case GGML_OP_SCALE:
            {
                ggml_compute_forward_scale(params, tensor->src[0], tensor->src1, tensor);
            } break;
        case GGML_OP_SET:
            {
                ggml_compute_forward_set(params, tensor->src[0], tensor->src1, tensor->opt[0], tensor);
            } break;
        case GGML_OP_CPY:
            {
        ...
        case GGML_OP_GET_ROWS:
            {
                ggml_compute_forward_get_rows(params, tensor->src[0], tensor->src1, tensor);
            } break;
        case GGML_OP_GET_ROWS_BACK:
            {
                ggml_compute_forward_get_rows_back(params, tensor->src[0], tensor->src1, tensor->opt[0], tensor);
            } break;
        case GGML_OP_DIAG:
            {
        ...
        case GGML_OP_DIAG_MASK_INF:
            {
                ggml_compute_forward_diag_mask_inf(params, tensor->src[0], tensor->src1, tensor);
            } break;
        case GGML_OP_DIAG_MASK_ZERO:
            {
                ggml_compute_forward_diag_mask_zero(params, tensor->src[0], tensor->src1, tensor);
            } break;
        ...
        case GGML_OP_SOFT_MAX_BACK:
            {
                ggml_compute_forward_soft_max_back(params, tensor->src[0], tensor->src1, tensor);
            } break;
        case GGML_OP_ROPE:
            {
                ggml_compute_forward_rope(params, tensor->src[0], tensor->src1, tensor);
            } break;
        case GGML_OP_ROPE_BACK:
            {
                ggml_compute_forward_rope_back(params, tensor->src[0], tensor->src1, tensor);
            } break;
        case GGML_OP_ALIBI:
            {
                ggml_compute_forward_alibi(params, tensor->src[0], tensor->src1, tensor);
            } break;
        case GGML_OP_CLAMP:
            {
                ggml_compute_forward_clamp(params, tensor->src[0], tensor->src1, tensor);
            } break;
        case GGML_OP_CONV_1D:
            {
                ggml_compute_forward_conv_1d(params, tensor->src[0], tensor->src1, tensor->opt[0], tensor);
            } break;
        case GGML_OP_CONV_2D:
            {
                ggml_compute_forward_conv_2d(params, tensor->src[0], tensor->src1, tensor->opt[0], tensor);
            } break;
        case GGML_OP_FLASH_ATTN:
            {
                ...
                ggml_compute_forward_flash_attn(params, tensor->src[0], tensor->src1, tensor->opt[0], masked, tensor);
            } break;
        case GGML_OP_FLASH_FF:
            {
                ggml_compute_forward_flash_ff(params, tensor->src[0], tensor->src1, tensor->opt[0], tensor->opt[1], tensor->opt[2], tensor);
            } break;
        case GGML_OP_FLASH_ATTN_BACK:
            {
                ...
                GGML_ASSERT(t == 0 || t == 1);
                bool masked = t != 0;
                ggml_compute_forward_flash_attn_back(params, tensor->src[0], tensor->src1, tensor->opt[0], tensor->opt[1], masked, tensor);
            } break;
        case GGML_OP_WIN_PART:
            {
                ggml_compute_forward_win_part(params, tensor->src[0], tensor->opt[0], tensor);
            } break;
        case GGML_OP_WIN_UNPART:
            {
                ggml_compute_forward_win_unpart(params, tensor->src[0], tensor->opt[0], tensor);
            } break;
        ...
        case GGML_OP_MAP_BINARY:
            {
                ...
                ggml_compute_forward_map_binary(params, tensor->src[0], tensor->src1, tensor, fun);
            }
            break;
        ...
        case GGML_OP_MAP_CUSTOM2:
            {
                ...
                ggml_compute_forward_map_custom2(params, tensor->src[0], tensor->src1, tensor, fun);
            }
            break;
        case GGML_OP_MAP_CUSTOM3:
            {
                const ggml_custom3_op_f32_t fun = *((ggml_custom3_op_f32_t *)tensor->opt[0]->data);
                ggml_compute_forward_map_custom3(params, tensor->src[0], tensor->src1, tensor->opt[1], tensor, fun);
            }
            break;
        case GGML_OP_CROSS_ENTROPY_LOSS:
            {
                ggml_compute_forward_cross_entropy_loss(params, tensor->src[0], tensor->src1, tensor);
            }
            break;
        case GGML_OP_CROSS_ENTROPY_LOSS_BACK:
            {
                ggml_compute_forward_cross_entropy_loss_back(params, tensor->src[0], tensor->src1, tensor->opt[0], tensor);
            }
            break;
        case GGML_OP_NONE:
            {
...
#if defined(GGML_USE_CUBLAS)
                        if (ggml_cuda_can_mul_mat(node->src[0], node->src1, node)) {
                            node->n_tasks = 1; // TODO: this actually is doing nothing
                                                //       the threads are still spinning
                        }
                        else
#elif defined(GGML_USE_CLBLAST)
                        if (ggml_cl_can_mul_mat(node->src[0], node->src1, node)) {
                            node->n_tasks = 1; // TODO: this actually is doing nothing
                                                //       the threads are still spinning
                            cur = ggml_cl_mul_mat_get_wsize(node->src[0], node->src1, node);
                        }
                        else
#endif
#if defined(GGML_USE_ACCELERATE) || defined(GGML_USE_OPENBLAS)
                        if (ggml_compute_forward_mul_mat_use_blas(node->src[0], node->src1, node)) {
                            node->n_tasks = 1; // TODO: this actually is doing nothing
                                               //       the threads are still spinning
                ...
                case GGML_OP_CROSS_ENTROPY_LOSS:
                    {
                        ...
                        size_t cur = ggml_type_size(node->type)*(node->n_tasks + node->src[0]->ne[0]*node->n_tasks);
                ...
                case GGML_OP_CROSS_ENTROPY_LOSS_BACK:
                    {
                        ...
                        size_t cur = ggml_type_size(node->type)*node->src[0]->ne[0]*node->n_tasks;
