...
static __global__ void pad_f32(const float * x, float * dst, const int ne0, const int ne00, const int ne01, const int ne02, const int ne03, const int p00, const int p10, const int p20) {
    ...
    if (nidx < ne00 + p00 && blockIdx.y < ne01 + p10 && nidx >= p00 && blockIdx.y >= p10 && blockIdx.z < (ne02*ne03) + p20 && blockIdx.z >= p20) {
        int offset_src =
            (nidx - p00) +
            (blockIdx.y - p10) * ne00 +
...
static void pad_f32_cuda(const float * x, float * dst,
    ...
    const int ne0, const int ne1, const int ne2, const int ne3, const int p00, const int p10, const int p20, cudaStream_t stream) {
