...
// quantize a model
bool replit_model_quantize(const std::string &fname_inp,
  ...
  // load vocab
  {
    ...
    printf("%s: loading vocab (%d words)\n", __func__, n_vocab);

    std::string word;
    for (int i = 0; i < n_vocab; i++) {
      printf("%s: loading vocab %d/%d\n", __func__, i + 1, n_vocab);
      uint32_t len;
...
int main(int argc, char **argv) {
  ...
  // load the model
  {
    ...
    if (!replit_model_quantize(fname_inp, fname_out, ggml_ftype(ftype))) {
